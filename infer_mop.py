...
def infer_analysis(agent, task_id, env, cfg, hidden_dim, action_dim):
    ...
    step, l1_sum, l2_sum = 0, np.zeros(hidden_dim), np.zeros(hidden_dim)
    ...
    for i in range(cfg.num_inference_episodes):
        ...
        while not time_step.last():
            ...
            with torch.no_grad():
                # Retrieve action
                a, activations = agent.infer_analysis(time_step.observation, task_id)
            # Execute action
            time_step = env.step(a)
            # Sum layer activations
            l1_sum += activations[0]
            l2_sum += activations[1]
            step += 1
...
def main(cfg):
    ...
    student = MOP(state_dim, action_dim, hidden_dim, cfg.hidden_layers, device, lr, ensemble)

    # Load student
    student_dir = work_dir / cfg.student_dir
    if cfg.data_sharing:
        ds = '_DS'
    student_dir = student_dir.resolve() / Path(f'{cfg.tasks[0]}_{cfg.data_type[0]}_{cfg.tasks[1]}_{cfg.data_type[1]}{ds}')
    student.load(student_dir)

    dict_list = []
    for idx in range(num_tasks):
        print(f'Inferencing task {cfg.tasks[idx]}')
        l1_avg, l2_avg = infer_analysis(student, idx, envs[idx], cfg, hidden_dim, action_dim)

        # Save results
        dict = {'l1': l1_avg, 'l2': l2_avg}
    ...
    tau_df = pd.DataFrame({'Layer 1': [tau_l1], 'Layer 2': [tau_l2]})
